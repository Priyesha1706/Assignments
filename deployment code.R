                      ...
                      tabItems(
                        ...
                        tabItem(tabName = "Input",
                                ...
                                fluidRow(
                                  ...
                                  box(title = "Delinq_2yrs",
                                      height = 130,width = 3 , background = "lime",
                                      numericInput("delinq_2yrs", label = h6(""), value = 3)),
                                  
                                  box(title = "inq_last_6mths",
                                      height = 130,width = 3, background = "olive",
                                      numericInput("inq_last_6mths", label = h6(""), value = 2)),
                                  
                                  box(title = "debt_income_ratio",
                                      height = 130,width = 3, background = "blue",
                                      numericInput("debt_income_ratio", label = h6(""), value = 14.5)),
                                  
                                  box(title = "numb_credit",
                                      height = 130,width = 3, background = "olive",
                                      numericInput("numb_credit", label = h6(""), value = 14)),
                                  
                                  box(title = "tot_curr_bal",
                                      height = 130,width = 3, background = "olive",
                                      numericInput("tot_curr_bal", label = h6(""), value = 28699)),
                                ...
                                fluidRow(
                                  ...
                                  box(title = "Fill Variables",  width = 3,  background = "red", height=120,
                                      ...
                                      selectInput(inputId = "five",label=h6(""),
                                                  choices = c("Rate_of_intrst","Experience","home_ownership","grade"),selected = "Rate_of_intrst")
                                  ),
...
server <- function(input, output) {
  ...
  test = reactive({
    if (input$submit > 0) {
      df <- data.frame(loan_amnt = (input$loan_amnt),Rate_of_intrst = (input$Rate_of_intrst),grade = (input$grade),
                       Experience = (input$Experience), home_ownership = (input$home_ownership),delinq_2yrs = (input$delinq_2yrs)
                       ,annual_inc = (input$annual_inc),inq_last_6mths = (input$inq_last_6mths),debt_income_ratio =(input$debt_income_ratio),numb_credit =(input$numb_credit),tot_curr_bal = (input$tot_curr_bal))
  ...
  fn2 = reactive({
    credit_data <- read.csv("C:\\Priyesha Data Science\\Project\\P28_balance\\data.csv")
    new_data <- credit_data %>% select(loan_amnt,Rate_of_intrst,annual_inc,home_ownership,delinq_2yrs,total.revol_bal,grade,inq_last_6mths,debt_income_ratio,numb_credit,tot_curr_bal)
    new_data <- new_data %>% mutate(Rate_of_intrst = as.numeric(sub("%","", Rate_of_intrst)),
                                    ...
                                    loan_amnt = as.numeric(loan_amnt),inq_last_6mths = as.numeric(inq_last_6mths))
  ...
  output$predt <- renderText({
    if (input$submit > 0) {
      ...
      credit_data_1 = credit_data[ ,c(2,5,6,9,10,11,16,17,15,20,36)]
